...
var SQLiteDB = function(sqlite3, dbSettings) {
  ...
  this.debug('Settings '+ JSON.stringify(dbSettings));
};
...
SQLiteDB.prototype.connect = function (callback) {
  ...
  if(self.client === undefined) {
    // location parameter for iOS:
    // 0=Documents (default, visible in iTunes, backed up by iCloud),
    // 1=Library (not visible in iTunes, backed up by iCloud,
    // 2=Library/LocalDatabase (not visible in iTunes, not backed up by iCloud))
...
SQLiteDB.prototype.debug = function () {
  if (this.debugMode) {
    ...
    args.unshift('[loopback-connector-cordova-sqlite] ');
...
SQLiteDB.prototype.executeSQL = function (sql, params, options, callback) {
  ...
  if (params.status && sql.startsWith('PRAGMA')) {
    self.client.transaction(function(tx) {
      tx.executeSql(sql, params, function success(rtx, result) {
        self.debug('['+sqlStmtIndx_+'] Success: '+sql+' called with params: ['+ params + ']');
        var new_rows = [];
        ...
        self.debug('['+sqlStmtIndx_+'] Result: '+JSON.stringify(new_rows));
    ...
    self.client.transaction(function (tx) {
      tx.executeSql(sql, params, function success(rtx, result) {
        ...
        if (sql.startsWith('UPDATE') || sql.startsWith('DELETE')) {
          result_ = {count: result.changes};
        } else {
          result_ = result.insertId;
        }
        self.debug('['+sqlStmtIndx_+'] Result: '+JSON.stringify(result_));
    ...
    self.client.transaction(function (tx) {
      tx.executeSql(sql, params, function success(rtx, result) {
        ...
        self.debug('['+sqlStmtIndx_+'] Result: '+JSON.stringify(data_));
