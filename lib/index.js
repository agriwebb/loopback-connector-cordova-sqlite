...
SQLiteDB.prototype.executeSQL = function (sql, params, options, callback) {
  ...
  if (params.status && sql.startsWith('PRAGMA')) {
    console.log("************ PRAGMA CALLED in loopback-connector-cordova-sqlite ************");
    ...
    self.client.transaction(function (tx) {
      tx.executeSql(sql, params, function success(rtx, result) {
        console.log('PASS ' + sqlStmtIndx_ + ': ' + sql);
        ...
        callback && callback(null, result_);
      }, function error(rtx, error) {
        console.error('FAIL ' + sqlStmtIndx_ + ': ' + sql);
    ...
    self.client.transaction(function (tx) {
      tx.executeSql(sql, params, function success(result) {
        console.log('PASS ' + sqlStmtIndx_ + ': ' + sql);
...
SQLiteDB.prototype.find = function find(model, id, callback) {
  ...
  this.query(sql, function (err, data) {
    ...
    callback && callback(err, this.fromDatabase(model, data[0]));
  }.bind(this));
};
