...
SQLiteDB.prototype.toDatabase = function (prop, val) {
  ...
  if (prop.type.name === 'ModelConstructor') {
    var retval;
...
SQLiteDB.prototype.create = function create(model, data, callback) {
  ...
  this.query(sql.join(''), generateQueryParams(data, props, true), function (err, lastUpdatedID) {
    if (err) {
...
SQLiteDB.prototype.save = function (model, data, callback) {
  ...
  self.query(sql.join(''), generateQueryParams(data, props, true), function (err) {
    callback(err);
  });
};
...
SQLiteDB.prototype.update =
    SQLiteDB.prototype.updateAll = function (model, where, data, options, callback) {
      ...
      this.query(sql, generateQueryParams(data, props, false), function (err, result) {
        if (callback) {
          var count;
...
SQLiteDB.prototype._buildWhere = function (model, conds) {
  ...
  // If model and conds have the model id defined then use it and return early
  var idColumnName = this.idName(model);
  if (idColumnName && conds[idColumnName] && typeof conds[idColumnName] === 'string') {
    return idColumnName + ' = \'' + conds[idColumnName] + '\'';
  }
...
function generateQueryParams(data, props, includeIds) {
  var queryParams = [];
