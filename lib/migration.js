    ...
    propNames.forEach(function (propName) {
      ...
      if (found && propertyHasNotBeenDeleted.call(self, model, propName)) {
        if (datatypeChanged(propName, found)) {
          sql.push('DROP COLUMN '+propName);
          sql.push('ADD COLUMN ' + modifyDatatypeInActual.call(self, model, propName));
        }
        if (nullabilityChanged(propName, found)) {
          sql.push('DROP COLUMN '+propName);
          sql.push('ADD COLUMN' + modifyNullabilityInActual.call(self, model, propName));
        }
      }
    });
    ...
    for (var p in data) {
      if(props[p]) {
        ...
        if (pType === 'GeoPoint' ||
            pType === 'Date'     ||
            pType === 'Boolean') {
          dbData[p] = this.toDatabase(props[p], data[p]);
        } else {
          dbData[p] = data[p];
        }
      }
